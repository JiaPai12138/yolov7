...
from numpy import uint8, uint32, frombuffer, ndarray
...
def read_data(buffer: ndarray):
    ...
    if not len(res_data):
        return None
    return loads(res_data.tobytes())  # tobytes()更快
...
def write_data_old(buffer: ndarray, data):
    buffer = buffer.reshape(-1).view(uint8)
    data_b = frombuffer(dumps(data, -1), dtype=uint8)
    buffer[:4] = np.array((len(data_b),), uint32).view(dtype=uint8)
...
class SharedStructure(SharedMemoryRecorder):
    ...
    def alloc(self, sm_name=None, create=True):
        ...
        try:
            ...
            self.sm = SharedMemory(sm_name, not create, self.__total_size)
        buffer = self.sm.buf
        offset = 0
        total_size = 0
